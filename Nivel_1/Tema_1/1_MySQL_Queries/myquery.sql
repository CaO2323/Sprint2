#Si us plau, efectua les següents consultes:
   ...
   SELECT producto.nombre, MIN(producto.precio), fabricante.nombre FROM producto INNER JOIN fabricante ON fabricante.codigo = producto.codigo_fabricante;
   #25 Retorna el nom del producte, el seu preu i el nom del seu fabricant, del producte més car.
   SELECT producto.nombre, MAX(producto.precio), fabricante.nombre FROM producto INNER JOIN fabricante ON fabricante.codigo = producto.codigo_fabricante;
   #26 Retorna una llista de tots els productes del fabricant Lenovo.
   SELECT producto.nombre, fabricante.nombre FROM producto INNER JOIN fabricante ON fabricante.codigo = producto.codigo_fabricante WHERE producto.codigo_fabricante = 2;
   ...
   SELECT producto.nombre, fabricante.nombre FROM producto INNER JOIN fabricante ON fabricante.codigo = producto.codigo_fabricante WHERE producto.codigo_fabricante IN (1,3,5);
   ...
   SELECT DISTINCT codigo_fabricante, fabricante.nombre FROM producto LEFT JOIN fabricante ON codigo_fabricante = fabricante.codigo;
   ...
   SELECT * FROM tienda.producto WHERE producto.precio = (SELECT MAX(producto.precio) FROM tienda.producto, tienda.fabricante WHERE fabricante.nombre = "Lenovo");
   #39 Llista el nom del producte més barat del fabricant Hewlett-Packard.
   SELECT producto.nombre, MIN(producto.precio) FROM producto INNER JOIN fabricante WHERE fabricante.nombre = "Hewlett-Packard";
...
SELECT apellido1, apellido2, persona.nombre, departamento.nombre FROM profesor INNER JOIN persona ON profesor.id_profColumnsesor = persona.id INNER JOIN departamento ON id_departamento = departamento.id ORDER BY apellido1, apellido2, persona.nombre ASC;
...
SELECT departamento.nombre, apellido1, apellido2, persona.nombre FROM universidad.persona INNER JOIN universidad.profesor ON profesor.id_profesor = persona.id LEFT JOIN universidad.departamento ON departamento.id = universidad.profesor.id_departamento ORDER BY departamento.nombre, persona.apellido1, persona.apellido2, persona.nombre;
...
SELECT apellido1, apellido2, persona.nombre FROM universidad.profesor LEFT JOIN universidad.persona ON profesor.id_profesor = persona.id LEFT JOIN asignatura ON profesor.id_profesor = asignatura.id_profesor WHERE asignatura.id_profesor IS NULL;
...
SELECT departamento.nombre, asignatura.id_grado FROM universidad.profesor INNER JOIN universidad.departamento ON departamento.id = profesor.id_departamento LEFT JOIN universidad.asignatura ON asignatura.id_profesor = profesor.id_profesor WHERE asignatura.id_grado IS NULL GROUP BY departamento.nombre;
#Consultes resum:
...
SELECT grado.nombre, asignatura.nombre FROM universidad.asignatura LEFT JOIN universidad.grado ON grado.id = asignatura.id_grado GROUP BY grado.nombre;
